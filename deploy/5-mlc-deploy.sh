...
##
# Example of use:
# /bin/bash ./5-mlc-deploy.sh 'nwaf_lic_key=xxx' 'api_srv_ip=x.x.x.x' 'fn_srv_ip=x.x.x.x' 'fn_rmq_user=xxx' 'fn_rmq_pwd=x.x.x.x' 'sys_proxy=xxx:xx' 'api_proxy=xxx:xx'
##
...
if [ -z "$fn_srv_ip" ]; then echo -e "\033[0;101mERROR: fn_srv_ip parameter is missing\033[0m" ; exit 1 ; fi
if [ -z "$fn_rmq_user" ]; then echo -e "\033[0;101mERROR: fn_rmq_user parameter is missing\033[0m" ; exit 1 ; fi
if [ -z "$fn_rmq_pwd" ]; then echo -e "\033[0;101mERROR: fn_rmq_pwd parameter is missing\033[0m" ; exit 1 ; fi

## Display the applied parameters
echo "Nemesida WAF license key: $nwaf_lic_key"
echo "Nemesida WAF API server IP: $api_srv_ip"
echo "Filtering node server IP: $fn_srv_ip"
echo "RabbitMQ user for connection to the filtering node: $fn_rmq_user"
echo "RabbitMQ password for connection to the filtering node: $fn_rmq_pwd"
...
sed -r "s|rmq_host = guest:guest@127.0.0.1|rmq_host = $fn_rmq_user:$fn_rmq_pwd@$fn_srv_ip|" /opt/mlc/mlc.conf
